...
def train(sess, model, data, show_plot):
    logs_path = './logs/'
    # log writer object to visualize experiment in tensorboard
    ...
    print( 'train accuracy before training: %6.4f' % \
            sess.run(model.accuracy, feed_dict={x: data['trX'], y:
                                                data['trY']}))
    ...
    print( 'test accuracy before training: %6.4f' % \
            sess.run(model.accuracy, feed_dict={x: data['vlX'], y:
                                                data['vlY']}))
    ...
    for i in xrange(epochs):
        for start, end in zip(range(0, len(data['trX']), batch_size), range(batch_size, len(data['trX']),batch_size)):
            _, summary = sess.run([model.train_op, model.summary_op], feed_dict={x: data['trX'][start:end], y: data['trY'][start:end]})
        ...
        if i % 50 == 0:
            print( "< Epoch%d >" % i)
            print( "Cross Entropy Cost: %6.4f" % (sess.run(model.cost,
                                                           feed_dict={x:
                                                                      data['trX'],
                                                                      y:
                                                                      data['trY']})))
            print( "Accuracy: ", sess.run(model.accuracy, feed_dict={x:
                                                                     data['vlX'],
                                                                     y:
                                                                     data['vlY']}))
...
def run_experiment():
    ...
    test_ids = data['Colour']
    # We want to search the hyperparameter space 
    sess = tf.Session()
    ...
    train(sess, model, data, show_plot = True)
