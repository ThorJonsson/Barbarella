...
class Seq2SeqModel(object):
  ...
  """
    ...
    def __init__(self,
               source_vocab_size, # our source is the same as the target
               target_vocab_size,
               buckets, # Needed for sequences of variable lengths
               size, # Number of hidden units
               num_layers, # Number of Hidden layers
               max_gradient_norm, # To monitor exploding gradients
               batch_size,
               learning_rate,
               learning_rate_decay_factor,
               num_samples=512, # What is this?
               forward_only=False, # To test our model
               dtype=tf.float32):
        """Create the model.
        ...
        self.target_vocab_size = target_vocab_size # We don't need this
        ...
        if num_samples > 0 and num_samples < self.target_vocab_size:
            ...
            softmax_loss_function = sampled_loss # We are reset the loss function
        rnn_cell = tf.nn.rnn_cell.GRUCell(size) # We use GRU for now
        ...
        for i in xrange(buckets[-1][1] + 1): #I'm pretty sure we don't need this
        ...
        # Here we can use encoder inputs
        targets = [self.decoder_inputs[i + 1]
                   for i in xrange(len(self.decoder_inputs) - 1)]

        # Training outputs and losses.
        if forward_only: # Modify to get states
            self.outputs, self.losses, self.states =\
                        tf.nn.seq2seq.model_with_buckets(self.encoder_inputs,\
                                                            self.decoder_inputs,\
                                                                        targets,\
                                                            self.target_weights,\
                                                                        buckets,\
        ...
        if output_projection is not None:
            for b in xrange(len(buckets)):# w = o_proj[0], b=o_proj[1]
                self.outputs[b] = [tf.matmul(output, output_projection[0]) + output_projection[1]\
                                   for output in self.outputs[b]]
        else: # Modify to get states
            self.outputs, self.losses,_ = tf.nn.seq2seq.model_with_buckets(self.encoder_inputs,\
                                        self.decoder_inputs,\
                                        targets,\
                                        self.target_weights,\
                                        buckets,\
    ...
    def step(self, session, encoder_inputs, decoder_inputs, target_weights,
        ...
        else: # Modify to get weights from outputs[-1]
